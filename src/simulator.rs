...
impl Simulator {
    ...
    pub fn update(&mut self, mut group_left: Vec<Character>, mut group_right: Vec<Character>) {
        group_left.iter_mut().for_each(|c| c.name.clear());
        group_right.iter_mut().for_each(|c| c.name.clear());
...
impl GroupData {
    fn apply_mod(&mut self, modification: CharModification) {
        ...
        let char = group.get_mut(index).expect("index does not exist!");
        modification(char);
    }
}
